...
import Shelf from '@/features/wallOfFame/Shelf';
import TakeBoard from '@/features/wallOfFame/TakeBoard';
  ...
  const takes = [
    {
      ...
      description: 'Called his rise to elite status before his breakout 2020 season',
      qbName: 'Josh Allen',
      date: 'Draft Day 2018',
    ...
    },
    {
      ...
      description: 'Predicted he would win Offensive Rookie of the Year over Tua',
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <div className="mb-6">
            ...
            <p className="text-xl text-white/70 max-w-2xl mx-auto">
              My crystal ball QB predictions and hot takes that stood the test of time
            </p>
          </div>
          
          <div className="flex justify-center items-center gap-8 text-white/50">
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-400">{takes.filter(t => t.status === 'correct').length}</div>
              <div className="text-sm">Correct Takes</div>
            </div>
            <div className="w-px h-12 bg-white/20"></div>
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-400">{successfulPredictions.length + breakoutPredictions.length}</div>
              <div className="text-sm">Crystal Ball QBs</div>
            </div>
            <div className="w-px h-12 bg-white/20"></div>
            <div className="text-center">
              <div className="text-2xl font-bold text-cyan-400">{Math.round((takes.filter(t => t.status === 'correct').length / takes.filter(t => t.status !== 'pending').length) * 100)}%</div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <section className="mb-16">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-white/90 mb-4">🔮 Crystal Ball Collection</h2>
            <p className="text-white/60 max-w-2xl mx-auto">
              Quarterbacks I had strong predictions about that came true. Each crystal ball represents a successful call.
            </p>
          </div>

          <div className="space-y-8">
            <Shelf 
              title="Elite Tier Predictions" 
              qbs={successfulPredictions}
            />
            
            <Shelf 
              title="Breakout Star Calls" 
              qbs={breakoutPredictions}
            />
          </div>
        </section>
